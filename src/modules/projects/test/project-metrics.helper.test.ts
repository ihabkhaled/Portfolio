...
describe('buildRepositoryMetricLabels', () => {
  it('returns an empty list when the snapshot is undefined', () => {
    expect(buildRepositoryMetricLabels(undefined, (count) => `${count} stars`)).toEqual([]);
  });
  ...
  it('includes only the fields present on the snapshot', () => {
    ...
    expect(buildRepositoryMetricLabels(snapshot, (count) => `${count} stars`)).toEqual([
      'TypeScript',
      'Apache-2.0',
    ]);
  });
