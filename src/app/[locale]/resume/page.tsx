...
import { ResumePageContainer } from '@/modules/resume';
...
export async function generateMetadata(properties: LocaleRouteProperties): Promise<Metadata> {
  ...
  return buildRouteMetadata({
    ...
    namespace: I18N_NAMESPACES.resume,
    titleKey: 'title',
    descriptionKey: 'description',
  });
}

export default async function ResumePage(properties: LocaleRouteProperties): Promise<ReactElement> {
  const { locale } = await properties.params;
  if (!isSupportedLocale(locale)) {
    appNotFound();
  }
  return <ResumePageContainer locale={locale} />;
}
