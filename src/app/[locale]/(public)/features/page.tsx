...
export async function generateMetadata(props: LocaleRouteProps): Promise<Metadata> {
  ...
  return isSupportedLocale(locale) ? buildMarketingMetadata(locale, 'features') : {};
}

export default async function FeaturesPage(props: LocaleRouteProps): Promise<ReactElement> {
  const { locale } = await props.params;
  if (!isSupportedLocale(locale)) {
    appNotFound();
  }
  return <MarketingPageContainer locale={locale} kind="features" />;
}
