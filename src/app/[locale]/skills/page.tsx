...
import { SkillsPageContainer } from '@/modules/skills';
...
export async function generateMetadata(properties: LocaleRouteProperties): Promise<Metadata> {
  ...
  return buildRouteMetadata({
    ...
    namespace: I18N_NAMESPACES.skills,
    titleKey: 'title',
    descriptionKey: 'description',
  });
}

export default async function SkillsPage(properties: LocaleRouteProperties): Promise<ReactElement> {
  const { locale } = await properties.params;
  if (!isSupportedLocale(locale)) {
    appNotFound();
  }
  return <SkillsPageContainer locale={locale} />;
}
