 ...
 */
export function AppQueryProvider(properties: Readonly<{ children: ReactNode }>): ReactElement {
  const [queryClient] = useState(makeQueryClient);

  return (
    <QueryClientProvider client={queryClient}>
      {properties.children}
      {publicEnvironment.appEnv === 'local' ? <ReactQueryDevtools initialIsOpen={false} /> : null}
    </QueryClientProvider>
  );
}
