...
| `next-intl`                                            | `src/packages/i18n`          | `useAppTranslation`, `useAppLocale`, `getServerTranslations`, `getServerLocale`, `AppIntlProvider`, `SUPPORTED_LOCALES` (17 locales), `DEFAULT_LOCALE`, `LOCALE_COOKIE_NAME` (`'NEXT_LOCALE'`), `getLocaleDirection`; one catalog per locale under `src/packages/i18n/messages/` |
...
| `next/headers`                                         | `src/packages/headers`       | `getRequestNonce` (reads the per-request CSP nonce `src/proxy.ts` stamped onto the request headers)                                                                                                                                                                              |
| `next/font/google`                                     | `src/shared/fonts`           | `appFontClassName` (app-fonts.ts) — bundles Space Grotesk (display), Inter (body), IBM Plex Mono (utility), and Noto Sans Arabic/Devanagari/Thai (script coverage), each selected per `:lang()` so a visitor only downloads what its locale renders                              |
...
| `nodemailer`                                           | `src/packages/mailer`        | `sendContactEmail` (server-only, guarded by `server-only`)                                                                                                                                                                                                                       |
