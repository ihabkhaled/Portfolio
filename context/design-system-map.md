...
| Feedback        | `Alert`, `Skeleton`, `Spinner`; shared `EmptyState`, `ErrorState`                           |
| Page chrome     | shared `SiteShell`, `PageHeader`, `SkipLink`, `VisuallyHidden`                              |
...
Before creating UI, inspect this map. Extend an existing variant when semantics match. Every new
interactive primitive needs a keyboard/focus contract, an accessible name, tests, dark-theme
tokens, and RTL-safe logical spacing.
